...
import 'package:quick_eats/blocs/restaurant_bloc/restaurant_event.dart';
...
class _RestaurantCardState extends State<RestaurantCard> {
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                  ...
                  image: DecorationImage(
                      image: widget.restaurant.picture_url != null
                          ?  NetworkImage(serverUrl +
                              widget.restaurant.picture_url)
  ...
  _showDialog(
      BuildContext context, Restaurant restaurant, ScopedQEModel model) {
    showDialog(
        context: context,
        builder: (BuildContext context) {
          return AlertDialog(
            ...
            content: Text(
                "Selecting a different restaurant clears the current cart. Do you wish to continue?"),
            actions: [
              FlatButton(
                child: Text("Yes"),
                onPressed: () {
                  ...
                  //BlocProvider.of<RestaurantBloc>(context).add(FetchMenuItems(restaurant.menus.elementAt(0).id));
                  Navigator.of(context).push(
                    MaterialPageRoute(
                        builder: (context) => BlocProvider<RestaurantBloc>(
                              child: RestaurantLanding(
  ...
  void _navigateToVendor(
    ...
    if (model.currentCart != null &&
        model.currentRestraurant != null &&
        model.currentRestraurant.restaurant_name !=
            restaurant.restaurant_name) {
      _showDialog(context, restaurant, model);
    } else {
      model.selectRestaurant(restaurant);
      //BlocProvider.of(context).add(FetchMenuItems);
      Navigator.of(context).push(
        MaterialPageRoute(
            builder: (context) => BlocProvider<RestaurantBloc>(
                  child: RestaurantLanding(
