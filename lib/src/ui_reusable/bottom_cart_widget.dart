...
class _ToolbarState extends State<BottomCartToolbar> {
  double getTotalPrice(ScopedQEModel model) {
    if (widget.option == CartOptions.continueShopping) {
      double price = widget.menuItem.item_price;
  ...
  void cartAction(ScopedQEModel model) {
    if (widget.option == CartOptions.cart) {
      Navigator.push(
          context, new MaterialPageRoute(builder: (context) => new CartPage()));
    } else {
      if (widget.option == CartOptions.continueShopping) {
          ...
          MaterialPageRoute(
              builder: (context) => BlocProvider<RestaurantBloc>(
                    child: RestaurantLanding(
                      restaurant: model.currentRestraurant,
                    ),
  ...
  void addToCart(ScopedQEModel model) {
    ...
    price += widget.extrasPrice;
    price += (price * (widget.numberItems - 1));
    cart.addToCart(new CartItem(widget.menuItem, widget.numberItems,
        items: widget.extraItems));
    model.populateCart(cart);
  }

  String getCartString(ScopedQEModel model) => widget.option == CartOptions.cart
      ? "Go to Cart"
      : widget.option == CartOptions.checkout
          ? "Check Out"
          : "Add ${widget.numberItems} to Cart";
}
