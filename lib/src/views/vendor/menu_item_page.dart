...
class _MenuItemPageState extends State<MenuItemPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
                ...
                child: Stack(
                  ...
                  children: <Widget>[
                    Container(
                      ...
                      child: FittedBox(
                        child: widget.item.item_picture_url != null
                            ? Image.network(
                                serverUrl + widget.item.item_picture_url)
                            : Image.asset("assets/images/waffle.jpg"),
                        fit: BoxFit.fitWidth,
                      ),
                    ),
                    HomeArrow(),
        ...
        bottomNavigationBar: ScopedModelDescendant<ScopedQEModel>(
            builder: (BuildContext context, Widget child, ScopedQEModel model) {
              //TODO add restaurant to menu item object
              //model.selectRestaurant(widget.item.restaurant);
          return buildAddToCartInfo(model);
        }));
  }
              ...
              Checkbox(
                  ...
                  onChanged: (checked) {
                    setState(() {
                      ...
                      totalExtrasPrice = checked
                          ? totalExtrasPrice + extraItem.price
                          : totalExtrasPrice - extraItem.price;
                    });
                  }),
              Text("${extraItem.name}"),
                    ...
                    children: <Widget>[
                      GestureDetector(
                          child: Icon(
                            Icons.remove,
                            color:
                                numberOfItems == 1 ? Colors.grey : Colors.green,
                            size: 25.0,
                          ),
                          onTap: () => setState(() {
                                if (numberOfItems > 1) {
